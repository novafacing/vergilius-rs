...
pub fn generate_bindings() -> Result<()> {
    ...
    let bindings_files = read_dir(&headers_dir)?
        ...
        .filter(|entry| entry.file_name().to_string_lossy().ends_with(".h") && entry.file_name().to_string_lossy().starts_with("bindings-10"))
    ...
    for bindings_file in bindings_files {
        ...
        let out_file = bindings_out_dir.join(&file_name).with_extension("rs");
